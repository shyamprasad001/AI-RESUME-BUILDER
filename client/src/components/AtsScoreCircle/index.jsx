...
function AtsScoreCircle({ score }) {
  ...
  const getColor = () => {
    ...
    if (score >= 60) return 'var(--brand-hover)';
    return 'var(--error)';
  };

  const getBgColor = () => {
    if (score >= 80) return 'var(--brand-light)';
    if (score >= 60) return 'var(--brand-soft)';
  ...
  return (
    <div className="score-circle-container">
      ...
      <span className="score-label" style={{ backgroundColor: getBgColor(), color: getColor() }}>
        {getLabel()}
      </span>
    </div>
  );
