...
const checkFormatting = (sections) => {
  ...
  results.sectionCompleteness = Math.round(
    (filled / Object.keys(checks).length) * 100,
  );

  const personal = sections.personalInfo || {};
  const contactFields = ["fullName", "email", "phone", "location", "linkedIn"];
  const contactFilled = contactFields.filter((f) =>
    (personal[f] || "").trim(),
  ).length;
  results.contactInfo = Math.round(
    (contactFilled / contactFields.length) * 100,
  );
  ...
  results.quantification =
    allBullets.length > 0
      ? Math.round(
          (allBullets.filter((b) => /\d/.test(b)).length / allBullets.length) *
            100,
        )
      : 0;
  ...
  if (wordCount >= 400 && wordCount <= 1200) results.length = 100;
    ...
    (wordCount > 1200 && wordCount <= 1600)
  )
    results.length = 70;
  else if (wordCount < 200)
    results.length = Math.max(0, Math.round((wordCount / 400) * 100));
  else
    results.length = Math.max(
      40,
      100 - Math.floor((wordCount - 1600) / 100) * 10,
    );
